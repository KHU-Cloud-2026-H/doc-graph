...
class RemoveWorkspaceMemberCommandHandlerTest {
    ...
    private fun member(workspaceId: Long, userId: Long): WorkspaceMember = WorkspaceMember(
        id = 1L,
    ...
    fun `정상 흐름 — delete 호출`() {
        ...
        val target = member(10L, 200L)
        every { memberRepository.findByWorkspaceIdAndUserId(10L, 200L) } returns target
