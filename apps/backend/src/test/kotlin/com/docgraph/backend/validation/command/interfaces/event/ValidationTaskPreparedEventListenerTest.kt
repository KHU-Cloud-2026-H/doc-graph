...
class ValidationTaskPreparedEventListenerTest @Autowired constructor(
    ...
    private fun wireFakes(edgeId: Long, sourceBlocks: List<Block>, targetBlocks: List<Block>) {
        ...
        findDocument.behavior = { id ->
            when (id) {
                sourceDocId -> DocumentDetail(id, "p-$id", "src", DocumentType.MEETING_NOTES, null, null, null, sourceBlocks)
                targetDocId -> DocumentDetail(id, "p-$id", "tgt", DocumentType.REQUIREMENTS, null, null, null, targetBlocks)
                else -> null
            }
        }
    }
