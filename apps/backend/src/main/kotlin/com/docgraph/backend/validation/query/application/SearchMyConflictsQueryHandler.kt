...
class SearchMyConflictsQueryHandler(
    ...
    override fun search(filter: MyConflictStatusFilter, pageable: Pageable): PageResponse<MyConflictRow> {
        ...
        val latestFindingByConflict = validationQueryRepository.findFindingsByConflictIds(conflictPage.content.map { it.id })
            .groupBy { it.conflictId }
            .mapValues { (_, findings) -> findings.maxWithOrNull(compareBy({ it.detectedAt }, { it.id })) }
        ...
        val rows = conflictPage.content.mapNotNull { conflict ->
            ...
            val latestFinding = latestFindingByConflict[conflict.id]
            MyConflictRow(
                id = conflict.id,
                ...
                sourceDocument = InboxDocumentRef(source.id, source.title, source.type, source.notionLastEditedAt),
                targetDocument = InboxDocumentRef(target.id, target.title, target.type, target.notionLastEditedAt),
                title = latestFinding?.title.orEmpty(),
                latestFindingId = latestFinding?.id,
                rationale = latestFinding?.rationale,
                targetBlockId = latestFinding?.targetBlockId,
                newText = latestFinding?.newText,
