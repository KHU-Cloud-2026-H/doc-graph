...
def test_uc8_type_assignee_routes_conflict_to_inbox(
    client, auth_headers, login, seeder, inbox_has, wait_until
):
    ...
    # 담당자 = default(admin)와 다른 user. 세션 발급(V17 FK) 위해 로그인으로 user_account 확보, /auth/me로 id 조회.
    assignee_headers = login("assignee")
    assignee_user_id = client.get("/auth/me", headers=assignee_headers).json()["id"]
    assignee_member_id = seeder.workspace_member(workspace_id=workspace_id, user_id=assignee_user_id)
